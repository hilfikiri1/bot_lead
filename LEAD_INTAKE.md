...
- Если рекомендованное действие — звонок: `Apply` сразу создаёт задачу
  «позвонить», кнопка `📞 Подготовка к звонку + контакт` показывает полный
  бриф (личный анализ, главный вопрос, сценарий на польском, шпаргалку),
  отправляет `.vcf` для iPhone и кнопку `📞 Позвонить`, а
  `✅ Поговорили` / `📵 Не ответил` / `📅 Перенести` / `❌ Номер неверный`
  сохраняют результат отдельным примечанием в Kommo.
