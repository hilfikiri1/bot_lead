"""Notion Data Sources gateway for the unified B&BS agent.

The legacy call-analysis Notion service remains available for old databases.
This gateway targets the Russian B&BS operational databases and is called only
after explicit manager confirmation for mutating operations.
"""
...
async def create_task(
    *,
    ...
    properties: dict[str, Any] = {
        ...
        "Sync status": {"select": {"name": "pending" if update_kommo else "synced"}},
        # A Notion task must never cause a second, hidden Kommo write. A future
        # sync worker may set this flag only from a separately approved action.
        "Обновить Kommo": {"checkbox": bool(update_kommo)},
    }
    if result:
...
async def sync_projects_from_kommo(
    ...
    *,
    ...
    """Upsert Kommo leads as operational projects after explicit approval."""
    created = 0
    updated = 0
    failed: list[dict[str, Any]] = []
    items: list[dict[str, Any]] = []
    for lead in leads[: max(1, min(limit, 100))]:
        try:
            page = await upsert_project_from_kommo(lead)
            created += int(bool(page.get("created")))
            updated += int(not bool(page.get("created")))
            items.append({"lead_id": lead.get("id"), **page})
        except Exception as exc:
            failed.append({"lead_id": lead.get("id"), "error": str(exc)[:500]})
    return {
        "created": created,
        "updated": updated,
        "failed": failed,
        "items": items,
        "processed": len(items) + len(failed),
    }
...
async def save_generated_draft(
    *,
    ...
    """Persist a generated draft to the matching operational Notion database."""
    project = await upsert_project_from_kommo(
        lead,
    ...
    if kind == "commercial_offer":
        record = await create_offer_draft(
            lead=lead, draft=draft, project_page_id=project.get("id")
        )
    elif kind == "catalog_outline":
        record = await create_catalog_draft(
            lead=lead, draft=draft, project_page_id=project.get("id")
        )
    else:
        record = await create_communication_draft(
            lead=lead, draft=draft, project_page_id=project.get("id")
        )
    external_id = (
        f"agent-draft:{kind or 'text'}:kommo:{int(lead['id'])}:"
        f"{datetime.now(timezone.utc).strftime('%Y%m%d%H%M')}"
    )
    task = await create_task(
        title=str(draft.get("title") or f"Проверить черновик по {lead.get('name')}")[:500],
        lead_id=int(lead["id"]),
        project_page_id=project.get("id"),
        priority="Средний",
        task_type=(
            "КП" if kind == "commercial_offer" else
            "Поставщик" if kind == "supplier_inquiry" else
            "Документы" if kind in {"catalog_outline", "technical_brief"} else
            "Сообщение"
        ),
    ...
    return {"project": project, "record": record, "task": task}
