...
This module intentionally sits beside the legacy new-lead summarizer. It enriches a
call with Kommo data, validates a strict AI decision, applies safe automatic CRM
changes, and keeps the client message as a draft only.
"""
...
_STAGE_ALIASES: dict[str, tuple[str, ...]] = {
    ...
    "Получено ТЗ": ("получено тз", "тз получено", "otrzymano specyfikację", "otrzymano tz"),
    "Поиск поставщиков": ("поиск поставщиков", "szukanie dostawców", "poszukiwanie dostawców"),
    "Получены предложения фабрик": (
        "получены предложения фабрик",
        "oferty fabryk otrzymane",
        "otrzymano oferty",
    ),
    "Подготовка расчёта": ("подготовка расчёта", "przygotowanie kalkulacji", "kalkulacja"),
    "Предложение отправлено": ("предложение отправлено", "oferta wysłana", "wysłano ofertę"),
    "Ожидание решения": ("ожидание решения", "oczekiwanie na decyzję"),
    "Образцы": ("образцы", "próbki"),
    "PI/договор": ("pi/договор", "pi", "umowa", "proforma"),
    "Ожидание оплаты": ("ожидание оплаты", "oczekiwanie na płatność", "czekamy na płatność"),
}
...
def normalize_polish_phone(value: str | None) -> str:
    ...
    if len(digits) == 9:
        return f"+48{digits}"
    return f"+{digits}" if value and str(value).strip().startswith("+") else digits
...
def clean_transcript(value: str) -> str:
    """Remove operator prompts, obvious ASR noise and exact repeated sentences."""
    clean = str(value or "").replace("\x00", " ")
    for pattern in _SYSTEM_PHRASES:
        clean = re.sub(pattern, " ", clean, flags=re.IGNORECASE)
    clean = re.sub(r"\[(?:noise|silence|музыка|шум|тишина)\]", " ", clean, flags=re.I)
...
async def _company_name(contact_id: int | None, fields: dict[str, str]) -> str:
    from_fields = _pick_field(fields, "company", "firma", "компания", "nazwa firmy")
    if from_fields:
        return from_fields
    if not contact_id:
        return ""
    try:
        contact = await kommo_service._request(
            "GET", f"/api/v4/contacts/{contact_id}", params={"with": "companies"}
        ) or {}
        refs = ((contact.get("_embedded") or {}).get("companies") or [])
        company_id = refs[0].get("id") if refs else None
        ...
        logger.info("Could not enrich Kommo company for contact %s: %s", contact_id, exc)
    return ""
...
async def build_lead_context(details: dict[str, Any]) -> LeadContext:
    ...
    fields = _custom_field_dict(details)
    contact_fields = {
        str(item.get("name") or item.get("code") or ""): str(item.get("value") or "")
        for item in contact.get("custom_fields") or []
        if item.get("value") not in (None, "")
    }
    all_fields = {**contact_fields, **fields}
    ...
    if tasks:
        ...
        if due:
            try:
                due_text = datetime.fromtimestamp(int(due), WARSAW).strftime("%d.%m.%Y")
            except (TypeError, ValueError, OSError):
                due_text = str(due)
        current_task = str(task.get("text") or "").strip()
        if due_text:
            current_task = f"{current_task} (до {due_text})".strip()
    ...
    return LeadContext(
        ...
        region=_pick_field(all_fields, "region", "województwo", "wojewodztwo", "область"),
        product_from_form=product,
        budget_from_form=budget,
        preferred_channel=_pick_field(
            all_fields, "kanał kontaktowy", "kanal kontaktowy", "channel", "канал"
        ),
        current_stage=str(details.get("status_name") or ""),
        current_priority=_pick_field(all_fields, "priority", "priorytet", "приоритет"),
...
async def _details_for_search(query: str) -> list[dict[str, Any]]:
    ...
    for item in result.get("leads") or []:
        ...
        if not isinstance(lead_id, int):
            continue
        try:
            details.append(await kommo_service.get_lead_details(lead_id))
        except Exception as exc:
            logger.info("Skipping Kommo candidate %s: %s", lead_id, exc)
    return details
...
async def resolve_lead_context(
    *,
    ...
    """Identify a Kommo deal in the required priority order."""
    if kommo_deal_id:
    ...
    if lead_id not in (None, ""):
        ...
        if lead_ref.isdigit():
            try:
                ...
                if exc.status_code != 404:
                    raise
        candidates = await _details_for_search(lead_ref)
        exact = [
            item
        ...
        if len(exact) == 1:
            return await build_lead_context(exact[0]), "lead_id", 1.0, []
        if len(exact) > 1:
            return None, "unresolved", 0.0, [_candidate(item) for item in exact]

    normalized_phone = normalize_polish_phone(phone)
    if normalized_phone:
        candidates = await _details_for_search(phone or normalized_phone)
        exact = []
        for item in candidates:
    ...
    if normalized_email:
        candidates = await _details_for_search(normalized_email)
        exact = []
        for item in candidates:
            values = [
    ...
    if helper_query:
        ...
        if len(candidates) == 1:
            return await build_lead_context(candidates[0]), "name_company", 0.85, []
        if len(candidates) > 1:
            return None, "unresolved", 0.0, [_candidate(item) for item in candidates]
...
def resolve_relative_due_date(transcript: str, call_date: date) -> date | None:
    ...
    explicit = re.search(r"\b(\d{1,2})[./-](\d{1,2})[./-](\d{4})\b", transcript)
    if explicit:
        try:
            return date(int(explicit.group(3)), int(explicit.group(2)), int(explicit.group(1)))
        except ValueError:
            pass
    if any(token in normalized for token in ("pojutrze", "послезавтра", "day after tomorrow")):
...
def _unsupported_fact(item: str, source: str) -> bool:
    ...
    for output_tokens, source_tokens in _FORBIDDEN_FACT_THEMES:
        if any(token in normalized_item for token in output_tokens) and not any(
            token in normalized_source for token in source_tokens
        ):
            return True
    return False
...
def _task_is_generic(title: str, description: str) -> bool:
    ...
    return len(combined) < 45


def _channel_label(context: LeadContext) -> str:
    channel = context.preferred_channel or "телефон"
...
def build_kommo_note(
    ...
    lines.extend(f"- {item}" for item in analysis.confirmed_in_call) or lines.append("- Нет новых подтверждений")
    lines.extend(["", "Новые данные:"])
    lines.extend(f"- {item}" for item in analysis.new_information) or lines.append("- Новых данных не зафиксировано")
    lines.extend(
        [
            "",
            "Ожидаем от клиента:",
            ...
            analysis.manager_commitment or analysis.kommo_update.task_description,
            "",
            "Следующий контакт:",
            (
...
def _postprocess_analysis(
    ...
    *,
    ...
    analysis.known_from_crm = list(dict.fromkeys(deterministic_known + analysis.known_from_crm))

    source = " ".join(
        [
            call_context.transcript,
            " ".join(analysis.known_from_crm),
            " ".join(lead_context.previous_notes),
        ]
    )
    filtered = False
    ...
    for item in analysis.confirmed_in_call:
        if _unsupported_fact(item, source):
            filtered = True
            continue
    ...
    if analysis.kommo_update.should_create_task:
        if due is None:
            ...
            analysis.review_reason = (
                analysis.review_reason + "; " if analysis.review_reason else ""
            ) + "Не удалось определить дату следующей задачи"
        if _task_is_generic(
            ...
            analysis.review_reason = (
                analysis.review_reason + "; " if analysis.review_reason else ""
            ) + "Модель предложила неконкретную следующую задачу"

    if filtered:
        analysis.needs_review = True
        analysis.review_reason = (
            analysis.review_reason + "; " if analysis.review_reason else ""
        ) + "Удалены неподтверждённые утверждения модели"

    if analysis.kommo_update.new_stage and _norm(analysis.kommo_update.new_stage) == _norm(
        lead_context.current_stage
    ):
        analysis.kommo_update.should_change_stage = False
        analysis.kommo_update.stage_reason = "Сделка уже находится на этой стадии"

    analysis.kommo_update.note = build_kommo_note(analysis, lead_context, call_context)
...
def _safe_review_analysis(
    *,
    ...
    return CRMCallAnalysis(
        ...
        manager_commitment="",
        waiting_for="other",
        priority=PriorityDecision(value="C", reason="Требуется ручная проверка анализа"),
        ...
        client_message=ClientMessageDraft(
            language="pl", channel="WhatsApp", text="", send_automatically=False
        ),
        needs_review=True,
...
def _resolve_stage_id(statuses: list[dict[str, Any]], canonical: str) -> tuple[int, str] | None:
...
async def _apply_kommo_actions(
    ...
    lead_id = int(lead_context.kommo_deal_id)

    if analysis.kommo_update.should_add_note and analysis.kommo_update.note:
        try:
            created = await kommo_service.add_common_note(
                lead_id, analysis.kommo_update.note
            )
            if created:
                completed.append(ActionCompleted(action="note_created", status="success"))
            else:
                completed.append(
                    ActionCompleted(
                        action="note_created", status="failed", error="Kommo did not confirm note creation"
                    )
                )
        except Exception as exc:
            logger.exception("Automatic Kommo note failed for lead %s", lead_id)
            completed.append(
                ActionCompleted(action="note_created", status="failed", error=str(exc)[:500])
            )

    if analysis.kommo_update.should_change_stage and analysis.kommo_update.new_stage:
        try:
            statuses = await kommo_service.get_pipeline_statuses(
                int(lead_context.pipeline_id or 0)
            )
            resolved = _resolve_stage_id(statuses, analysis.kommo_update.new_stage)
            if not resolved:
                completed.append(
                    ActionCompleted(
                        ...
                        new_value=analysis.kommo_update.new_stage,
                        error="Requested stage does not exist in the current Kommo pipeline",
                    )
                )
            else:
                status_id, status_name = resolved
                await kommo_service.update_kommo_lead(lead_id, status_id=status_id)
                verified = await kommo_service.get_lead_details(lead_id)
            ...
            logger.exception("Automatic Kommo stage update failed for lead %s", lead_id)
            completed.append(
                ActionCompleted(
                    action="stage_updated",
                    status="failed",
    ...
    if update.should_create_task and update.task_due_date:
        ...
        try:
            ...
            existing_tasks = await kommo_service.get_open_lead_tasks(lead_id, limit=20)
            if existing_tasks and isinstance(existing_tasks[0].get("id"), int):
                ...
                verified_tasks = await kommo_service.get_open_lead_tasks(lead_id, limit=20)
                verified = next(
                    (item for item in verified_tasks if int(item.get("id") or 0) == task_id),
                    None,
                )
                if not verified or int(verified.get("complete_till") or 0) != timestamp:
            ...
            logger.exception("Automatic Kommo task update failed for lead %s", lead_id)
            completed.append(
                ActionCompleted(
                    action="task_created",
                    status="failed",
...
def to_legacy_analysis(
    ...
    priority_to_urgency = {"A1": "high", "A2": "high", "B": "medium", "C": "low", "D": "low"}
    ...
    payload.update(
        {
            "client": {
                ...
                "kommo_contact_id": int(context.contact_id) if context.contact_id.isdigit() else None,
            },
            "lead": {
                "lead_number": context.lead_id or None,
                "proposed_name": context.lead_name or context.product_from_form or "Разговор с клиентом",
            ...
            "recommended_next_step": analysis.kommo_update.task_description
            ...
            "calendar": {
                "title": analysis.kommo_update.task_title or "Следующий контакт",
            ...
            "actions_completed": [item.model_dump(mode="json") for item in analysis.actions_completed],
        }
    )
    return payload
    ...
    *,
    ...
    client = await crm_service.upsert_client(
        db,
        {
            ...
            "kommo_contact_id": (
                int(lead_context.contact_id) if lead_context.contact_id.isdigit() else None
            ),
            "source": "kommo_call_analysis",
        },
    )
    result = await db.execute(
        select(Lead).where(Lead.kommo_lead_id == int(lead_context.kommo_deal_id or 0))
    )
    lead = result.scalars().first()
    priority = {"A1": "high", "A2": "high", "B": "medium", "C": "low", "D": "low"}[
    ...
    if lead is None:
        lead = await crm_service.create_lead(
            ...
            client,
            {
                ...
                "country": "Poland" if lead_context.phone.startswith("+48") else None,
                "city": lead_context.region,
                "status": "follow_up",
                "urgency": "high" if priority == "high" else "medium" if priority == "medium" else "low",
                "next_action": analysis.kommo_update.task_description,
                "next_followup_at": analysis.kommo_update.task_due_date,
            },
        )
    else:
        lead.client_id = client.id
        lead.product_requested = lead_context.product_from_form or lead.product_requested
        lead.budget = lead_context.budget_from_form or lead.budget
        lead.city = lead_context.region or lead.city
        lead.priority = priority
        lead.next_action = analysis.kommo_update.task_description or lead.next_action
        lead.next_followup_at = analysis.kommo_update.task_due_date
        await db.commit()
...
def format_actions_report(analysis: CRMCallAnalysis) -> str:
    if analysis.needs_review:
        return (
            ...
            f"Причина: {analysis.review_reason}"
        )
    ...
    for action in analysis.actions_completed:
        if action.status == "success":
            ...
            if action.new_value and action.action == "stage_updated":
                suffix = f": {action.old_value or '—'} → {action.new_value}"
            if action.due_date:
                suffix += f" · {action.due_date.strftime('%d.%m.%Y')}"
            lines.append(f"• {labels[action.action]}{suffix}")
        else:
            lines.append(f"• ❌ {labels[action.action]}: {action.error or action.status}")
...
async def process_completed_call(
    *,
    ...
    payload = CRMCallInput(lead_context=lead_context, call_context=call_context)
    try:
    ...
    if confidence < 0.95:
        ...
        analysis.review_reason = (
            analysis.review_reason + "; " if analysis.review_reason else ""
        ) + "Идентификация основана только на имени/компании"

    analysis.actions_completed = await _apply_kommo_actions(lead_context, analysis)
    return CRMCallProcessingResult(
        lead_context=lead_context,
        call_context=call_context,
        analysis=analysis,
