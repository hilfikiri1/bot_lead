...
def upgrade() -> None:
    op.create_table(
        ...
        sa.UniqueConstraint("client_message_draft_id", name="uq_whatsapp_client_message_draft_id"),
    )
    for column in (
        "provider_message_id", "client_message_draft_id", "kommo_lead_id",
        "direction", "status", "phone", "phone_number_id",
        "provider_timestamp", "created_at",
    ):
        op.create_index(
            f"ix_whatsapp_cloud_messages_{column}",
            "whatsapp_cloud_messages",
            [column],
        )
